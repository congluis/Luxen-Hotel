    ...
    var f,
      ...
      h = {
          ...
          return (
            ...
            this.each(function () {
              if (a(this).parent().hasClass("sod_select"))
                console.log(
                  "Select or Die: It looks like the SoD already exists"
                );
              else {
                var u,
                  v,
                  ...
                  e = b.data("custom-class")
                    ? b.data("custom-class")
                    : d.customClass,
                  f = b.data("prefix") ? b.data("prefix") : d.prefix,
                  g = b.data("placeholder")
                    ? b.data("placeholder")
                    : d.placeholder,
                  i = b.data("placeholder-option")
                    ? b.data("placeholder-option")
                    : d.placeholderOption,
                  j = b.data("cycle") ? b.data("cycle") : d.cycle,
                  k = b.data("links") ? b.data("links") : d.links,
                  l = b.data("links-external")
                    ? b.data("links-external")
                    : d.linksExternal,
                  m = parseInt(b.data("size")) ? b.data("size") : d.size,
                  n = parseInt(b.data("tabindex"))
                    ? b.data("tabindex")
                    : d.tabIndex
                    ? d.tabIndex
                    : b.attr("tabindex")
                ...
                f && (r = '<span class="sod_prefix">' + f + "</span> "),
                  ...
                  (v = a("<span/>", { class: "sod_list_wrapper" }).appendTo(u)),
                  (w = a("<span/>", { class: "sod_list" }).appendTo(v)),
                  a("option, optgroup", b).each(function (b) {
                    var c = a(this);
                    o && !a.trim(c.text())
                      ? c.remove()
                      : 0 === b && i && !r
                      ? h.populateSoD(c, w, u, !0)
                      : h.populateSoD(c, w, u, !1);
                  }),
                  m &&
                    (v.show(),
                  ...
                  b.appendTo(u),
                  u
                    .on("focusin", h.focusSod)
                    .on("click", h.triggerSod)
                    .on("click", ".sod_option", h.optionClick)
                    .on("mousemove", ".sod_option", h.optionHover)
                    .on("keydown", h.keyboardUse),
            ...
            ? (a("<span/>", {
              ...
              e && !h
                ? (d.data("label", k),
                  ...
                  p && i.addClass("sod_placeholder"))
                : p && f && !g && !h
                ? d.data("label", f)
                : p && d.data("label", k),
              ((p && !f) || (p && g) || (p && h)) && i.append(k),
              j.is("optgroup") &&
                (c.find(".sod_option:last").addClass("groupchild"),
                j.is(":disabled") &&
                  c.find(".sod_option:last").addClass("disabled")))
            : a("<span/>", {
        ...
        focusSod: function () {
          ...
          b.hasClass("disabled")
            ? h.blurSod(b)
            : (h.blurSod(a(".sod_select.focus").not(b)),
              b.addClass("focus"),
        ...
        triggerSod: function (b) {
          ...
          var c = a(this),
            ...
            f = c.find(".active"),
            i = c.find(".selected");
          c.hasClass("disabled") || c.hasClass("open") || c.hasClass("touch")
            ? (clearTimeout(g),
              c.removeClass("open"),
              e && (c.find(".sod_label").get(0).lastChild.nodeValue = f.text()))
            : (c.addClass("open"),
              e &&
                !c.data("prefix") &&
                c.find(".sod_label").addClass("sod_placeholder").html(e),
              h.listScroll(d, i),
              h.checkViewport(c, d));
        },
        keyboardUse: function (b) {
          var l,
            ...
            j = c.data("cycle"),
            k = g.filter(".active");
          return b.which > 36 && b.which < 41
            ? (37 === b.which || 38 === b.which
                ? ((m = k.prevAll(":not('.disabled, .optgroup')").first()),
                  (n = g.not(".disabled, .optgroup").last()))
                : (39 === b.which || 40 === b.which) &&
                  ((m = k.nextAll(":not('.disabled, .optgroup')").first()),
                  (n = g.not(".disabled, .optgroup").first())),
              !m.hasClass("sod_option") && j && (m = n),
              (m.hasClass("sod_option") || j) &&
            ...
            : (13 === b.which ||
              (32 === b.which &&
                c.hasClass("open") &&
                (" " === c.data("filter")[0] || "" === c.data("filter")))
                ? (b.preventDefault(), k.click())
                : 32 !== b.which ||
                  c.hasClass("open") ||
                  (" " !== c.data("filter")[0] && "" !== c.data("filter"))
                ? 27 === b.which && h.blurSod(c)
                : (b.preventDefault(), (e = !1), c.click()),
              0 !== b.which &&
                (clearTimeout(f),
                c.data(
                  "filter",
                  c.data("filter") + String.fromCharCode(b.which)
                ),
                (l = g
                  .filter(function () {
                    return (
                      ...
                      a(this)
                        .text()
                        .toLowerCase()
                        .indexOf(c.data("filter").toLowerCase())
                    );
                  })
        ...
        selectChange: function () {
          ...
          (f.find(".sod_label").get(0).lastChild.nodeValue = e),
            ...
            (!f.data("links") && !c.data("link")) || c.data("link-external")
              ? (f.data("links-external") || c.data("link-external")) &&
                window.open(c.val(), "_blank")
        ...
        blurSod: function (b) {
          if (a("body").find(b).length) {
            var c = b.data("label"),
              ...
              i = !1;
            clearTimeout(g),
              e && !f.hasClass("selected")
                ? (f.click(), (i = !0))
                : f.hasClass("selected") ||
                  (f.removeClass("active"), h.addClass("active")),
              !i && d
                ? (b.find(".sod_label").get(0).lastChild.nodeValue = h.text())
                : i || (b.find(".sod_label").get(0).lastChild.nodeValue = c),
              (e = !1),
        ...
        checkViewport: function (b, c) {
          var d = b[0].getBoundingClientRect(),
            e = c.outerHeight();
          d.bottom + e + 10 > a(window).height() && d.top - e > 10
            ? b.addClass("above")
            : b.removeClass("above"),
        ...
        listScroll: function (a, b) {
          ...
          c.top > d.top
            ? a.scrollTop(a.scrollTop() - c.top + d.top)
            : c.bottom < d.bottom &&
              a.scrollTop(a.scrollTop() - c.bottom + d.bottom);
        },
        isTouch: function () {
          return (
        ...
        destroy: function () {
          return this.each(function () {
            ...
            c.hasClass("sod_select")
              ? (b.off("change"), c.find("span").remove(), b.unwrap())
        ...
        disable: function (b) {
          return this.each(function () {
            ...
            d.hasClass("sod_select")
              ? "undefined" != typeof b
                ? (d
                    .find(".sod_list:first .sod_option[data-value='" + b + "']")
                    .addClass("disabled"),
                  d
                    .find(".sod_list:first .sod_option[data-label='" + b + "']")
                    .nextUntil(":not(.groupchild)")
                    .addClass("disabled"),
                  a(
                    "option[value='" + b + "'], optgroup[label='" + b + "']",
                    this
                  ).prop("disabled", !0))
                : d.hasClass("sod_select") &&
                  (d.addClass("disabled"), c.prop("disabled", !0))
          ...
          return this.each(function () {
            ...
            d.hasClass("sod_select")
              ? "undefined" != typeof b
                ...
                : d.hasClass("sod_select") &&
                  (d.removeClass("disabled"), c.prop("disabled", !1))
